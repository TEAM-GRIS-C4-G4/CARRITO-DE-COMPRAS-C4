        ...
        <form id="department">
            <fieldset>
                <div class="alert alert-dismissible alert-warning">
                    <p class="mb-0">Registro de Categorias</p>
                </div>
...
export default {
    ...
    methods: {
        loadData: function () {
            axios.get('http://150.136.125.111:8080/api/categories').then(result => {
                this.listData = result.data
            })
        },
        findByid: function (id) {
            // metodo para consutlar por el ig del boton impreso en la vista
            axios.get('http://150.136.125.111:8080/api/categories/' + id).then(result => {
            ...
            if (bandera == true) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete('http://150.136.125.111:8080/api/categories/' + id).then(() => {
                            Swal.fire({
                                icon: 'success',
        ...
        dataAdd: function () {
            ...
            axios.post('http://150.136.125.111:8080/api/categories', data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
        ...
        dataUpdate: function () {
            ...
            axios.put('http://150.136.125.111:8080/api/categories/' + this.id, data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
                        title: "'El registro se guardó de forma correcta.'",
