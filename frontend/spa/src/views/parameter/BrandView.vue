        ...
        <form id="department">
            <fieldset>
                <div class="alert alert-dismissible alert-warning">
                    <p class="mb-0">Registro de Marcas</p>
                </div>
...
export default {
    ...
    methods: {
        loadData: function () {
            axios.get('http://150.136.125.111:8080/api/brands').then(result => {
                this.listData = result.data
            })
        },
        findByid: function (id) {
            // metodo para consutlar por el ig del boton impreso en la vista
            axios.get('http://150.136.125.111:8080/api/brands/' + id).then(result => {
            ...
            if (bandera == true) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete('http://150.136.125.111:8080/api/brands/' + id).then(() => {
                            Swal.fire({
                                icon: 'success',
        ...
        dataAdd: function () {
            ...
            axios.post('http://150.136.125.111:8080/api/brands', data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
        ...
        dataUpdate: function () {
            ...
            axios.put('http://150.136.125.111:8080/api/brands/' + this.id, data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
                        title: "'El registro se guardó de forma correcta.'",
